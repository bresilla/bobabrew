...
PROJECT_NAME := bobabrew
NIMBLE_FILE := bobabrew.nimble
...
build:
	@$(NIM) c $(BUILD_FLAGS) --out:$(PROJECT_NAME) src/bobabrew.nim
...
test-all:
	@$(NIMBLE) test -y
	@$(NIM) c -d:release --out:/tmp/bobabrew-release-check src/bobabrew.nim
	@rm -f /tmp/bobabrew-release-check
...
fmt-check:
	...
	for f in $(NIM_FILES); do \
		tmp="/tmp/bobabrew-nimpretty-$$(basename "$$f").$$$$.nim"; \
		cp "$$f" "$$tmp"; \
